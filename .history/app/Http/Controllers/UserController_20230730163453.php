...
class UserController extends Controller
{
    ...
    public function index3(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('filter_country')) {
                $filterCountry = $request->input('filter_country');
                $data->where('countries.name', 'LIKE', '%' . $filterCountry . '%');
            }
    ...
    if ($request->has('filter_country')) {
        $users->whereHas('filter_country', function ($query) use ($request) {
            $query->where('country.name', $request->input('filter_country'));
        });
    }
    // Apply date filter if provided
    if ($request->has('filter_date')) {
    ...
    return DataTables::of($users)
        ->addColumn('country.name', function ($user) {
            return $user->country->na
            dd($user->country->name);
        })
        ->rawColumns(['country.name'])
        ->make(true);
    // }
return view('users.index', compact('countries'));
    }
