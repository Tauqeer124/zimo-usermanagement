...
$query = User::with('country', 'userDetail')->where('country_id', $countryId)
->orderBy('id', 'asc')->get;


if ($request->input('country_id') ) {
    // If the country_id is provided, sort by ascending id
    $query->where('country_id', $countryId)
          ->orderBy('id', 'asc');
} else {
    // If no country_id is provided, keep the original descending order
    $query->orderBy('id', 'desc');
}

$users = $query->get();
dd($users);// Return the results as a UserResource collection
    return UserResource::collection($users);
}
    /**
