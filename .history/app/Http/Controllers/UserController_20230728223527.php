...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('filter_country')) {
                ...
                $countries->where('name', 'LIKE', '%' . $filterCountry . '%');
            }
     ...
     * @return \Illuminate\Contracts\View\View
     */
    public function showuserdata(Request $request, $name)
    {
        // Fetch users from the specific country

        // $users = Country::where('name', $name)->get();
        $usersByCountry = User::select('users.country_id', 'countries.name ')
        ->leftJoin('countries', 'users.country_id', '=', 'countries.id')->
        select(DB::raw('countries.name, count(*) as count'))->groupby('name')->get();
        dd($users);
