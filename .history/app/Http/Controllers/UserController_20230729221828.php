...
class UserController extends Controller
{
    ...
    public function index2(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('filter_country')) {
                ...
                $countries->where('countries.name', 'LIKE', '%' . $filterCountry . '%');
            }
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = User::with('country')
                ->when($request->has('filter_country'), function ($query) use ($request) {
                    $filterCountry = $request->input('filter_country');
                    $query->whereHas('country', function ($query) use ($filterCountry) {
