...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $users = User::with('country')->select('users.*');
    ...
    public function store(Request $request)
    {
        ...
        $user = new User();
        $user->name = $userdata->input('name');
        $user->email = $userdata->input('email');
        $user->phone = $userdata->input('phone');
        $user->country_id = $userdata->input('country_id');
        $user->password = Hash::make($userdata->input('password'));
        ...
        if ($request->hasFile('image')) {
            ...
            $path = Storage::putFile('user_images', $image); // Upload image to Firebase storage
            $userDetails->image_url = Storage::url($path); // Save the Firebase URL in the database
            $userDetails->save();
        }
