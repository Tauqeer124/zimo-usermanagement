              ...
              
{
    ...
    render: function (data, type, row) {
                    var statusLabel = data === 'active' ? 'Active' : 'Block';
                    var statusClass = data === 'active' ? 'btn-success' : 'btn-danger';
                    return '<button class="btn btn-sm status-btn ' + statusClass + '" data-user-id="' + row.id + '" data-status="' + data + '">' + statusLabel + '</button>';
                }
            },
                {
                    data: null,
    ...
    $.ajax({
        ...
        success: function (data) {
            // Update the button label and class based on the updated status
            var statusLabel = newStatus === 'active' ? 'Active' : 'Block';
            var statusClass = newStatus === 'active' ? 'btn-success' : 'btn-danger';
            button.data('status', newStatus);
            button.text(statusLabel);
            button.removeClass('btn-success btn-danger').addClass(statusClass);
        },
        error: function (xhr) {
            // Handle error messages if needed
        }
